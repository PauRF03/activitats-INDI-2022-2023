...
//Crear la TG del cap i cos
void MyGLWidget::modelTransformCapICos() {
	glm::mat4 TG(1.0f);
	TG = glm::rotate(TG, glm::radians((float)angler), glm::vec3(0, 1, 0));
	TG = glm::scale(TG,glm::vec3(0.4,0.4,1.0));  
	glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]);
}

//Crear la TG de la cama dreta
...
//Crear la TG de la cama esquerra
...
//Crear la TG del braç dret
void MyGLWidget::modelTransformBracDret() {
	glm::mat4 TG(1.0f);
	TG = glm::rotate(TG, glm::radians((float)angler), glm::vec3(0, 1, 0));
	if(corona == 2){
		TG = glm::translate(TG,glm::vec3(-0.15, 0.15, 0.0));
		TG = glm::rotate(TG, glm::radians(240.f), glm::vec3(0, 0, 1));
		TG = glm::translate(TG,glm::vec3(0.0, -0.2, 0.0));
	}else TG = glm::translate(TG,glm::vec3(-0.15, -0.05, 0.0));
	TG = glm::scale(TG,glm::vec3(0.2, 0.2, 1.0));
	glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]);
}

//Crear la TG del braç esquerre
...
//Crear la TG de la corona
void MyGLWidget::modelTransformCorona() {
	glm::mat4 TG(1.0f);
	TG = glm::rotate(TG, glm::radians((float)angler), glm::vec3(0, 1, 0));
	TG = glm::translate(TG,glm::vec3(0, 0.7, 0.0));
	TG = glm::scale(TG,glm::vec3(0.2, 0.2, 1.0));
	glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]); 
}
...
void MyGLWidget::carregaShaders() {
	...
	difLoc = glGetUniformLocation(program -> programId(), "dif");
}
