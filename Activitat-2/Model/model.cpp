...
static void omplenormals(vector<Face> &_faces, vector<Vertex> const &_vertices);
static void ompleVBOs(vector<Face> &_faces, vector<Vertex> const &_vertices, vector<Normal> const &_normals, float *&_VBO_vert, float *&_VBO_norm, float *&_VBO_mata, float *&_VBO_matd, float *&_VBO_matsp, float *&_VBO_matsh);
...
void Model::load(std::string filename) {
	...
	ompleVBOs(_faces, _vertices, _normals, _VBO_vertices, _VBO_normals, _VBO_matamb, _VBO_matdiff, _VBO_matspec, _VBO_matshin);
}
...
void Model::dumpModel() const {
	...
	for (unsigned int i = 0; i < _faces.size(); ++i) {
		cout << "f";
		if (_faces[i].n.empty()){
			for (int j = 0; j < 3; ++j) cout << " " << _faces[i].v[j]/3 + 1;
			cout << endl;
		} else {
			for (int j = 0; j < 3; ++j) cout << " " << _faces[i].v[j]/3 + 1 << "//" << _faces[i].n[j]/3 + 1;
			cout << endl;      
		}
	}
}
...
static void loadMTL(std::string filename) {
	...
	while (getline(input, line)) {
		...
		if (wrd[0] == '#') ;
			...
			cerr << "Processing '" << wrd << "'" << endl;
			#endif
			for (int i = 0; i < 3; ++i) ss >> Materials.back().ambient[i];
		} else if (wrd == "Kd") {
			#if DEBUGPARSER
			cerr << "Processing '" << wrd << "'" << endl;
			#endif
			for (int i = 0; i < 3; ++i) ss >> Materials.back().diffuse[i];
		} else if (wrd == "Ks") {
			#if DEBUGPARSER
			cerr << "Processing '" << wrd << "'" << endl;
			#endif
			for (int i = 0; i < 3; ++i) ss >> Materials.back().specular[i];
		} else {
			#if DEBUGPARSER
...
static void omplenormals(vector<Face> &_faces, const vector<Vertex>  &_vertices) {
	for (unsigned int i = 0; i < _faces.size(); ++i) {
		double v0[3], v1[3];
		int P0 =_faces[i].v[0];
		int P1 =_faces[i].v[1];
...
static void ompleVBOs(vector<Face> &_faces, const vector<Vertex> &_vertices, const vector<Normal> &_normals, float *&_VBO_vert, float *&_VBO_norm, float *&_VBO_mata, float *&_VBO_matd, float *&_VBO_matsp, float *&_VBO_matsh) {
	...
	for (unsigned int f = 0; f < _faces.size(); ++f) {
		...
		for (int i = 0; i < 3; ++i) {
			...
			for (int j = 0; j < 3; ++j) {
				...
				if (_normals.size() != 0) _VBO_norm[index+j] = _normals[_faces[f].n[i]+j];
				else _VBO_norm[index+j] = _faces[f].normalC[j];	
