...
void MyGLWidget::viewTransform() {
	...
	if(POVMorty == 0){
  	...
  	}else View = glm::lookAt(posMorty + glm::vec3(0, 0.8, 0), glm::vec3(100 * sin(glm::radians(angleMorty)), 0.8, 100 * cos(glm::radians(angleMorty))), glm::vec3(0, 1, 0));
    		...
    		case Qt::Key_E:
      		...
      		case Qt::Key_P:
        		if(fmod(angleRick, 360.0) == fmod(anglePortal - 180.0, 360.0)){
            			if(pintarPortal == 0) pintarPortal = 1;
            			else pintarPortal = 0;
            		}else{
...
void MyGLWidget::canviacamera(int value) {
	makeCurrent();
	POVMorty = value;
	if(POVMorty == 1) znear = 0.25;
	else znear = distancia - radiEscena;
	viewTransform();
	projectTransform();
	update();
}
